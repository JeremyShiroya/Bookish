...
export const useBooks = () => {
  ...
  const recentAuthors = computed(() => {
    return authors.value.slice(0, 4);
  });
  ...
  const updateBook = async (updatedBook) => {
    ...
    try {
      ...
      if (index !== -1) {
        ...
        // Refresh all data to sync genres, authors, and playlists.
        fetchAllData(true);
      }
    } catch (error) {
  ...
  const createPlaylist = async (playlist) => {
    try {
      const savedPlaylist = await $fetch("/api/collections", {
        method: "POST",
        body: playlist,
      });
      collections.value = [savedPlaylist, ...collections.value];
      await fetchAllData(true);
  ...
  const addBookToPlaylist = async (playlistId, bookId) => {
    try {
      await $fetch(`/api/collections/${playlistId}/books`, {
        method: "POST",
        body: { bookId },
      });
      await fetchAllData(true);
    } catch (error) {
      console.error("Failed to add book to playlist:", error);
      throw error;
    }
  };
  ...
  const popularBooks = computed(() => {
    return [...books.value]
      .sort((a, b) => {
        const ratingDifference = getGoodreadsRating(b) - getGoodreadsRating(a);
